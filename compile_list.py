...
def add_new_papers(existing_papers):
    folder = "./data/papers/"
    paper_names = os.listdir(folder)
    for paper_name in paper_names:
        print(paper_name)
...
def write_papers(papers, final_file=None, header_file=None, end_file=None, TYPE='md'):
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='Compile neuroEthology papers.')
    ...
    old_final_file = os.path.join('previosly_compiled_papers.%s' % TYPE)
    # load papers
    ...
    with open(out_file, 'w') as f:
        md_content = write_papers(papers, old_final_file, header_file, end_file, TYPE=TYPE)
        f.write(md_content)
